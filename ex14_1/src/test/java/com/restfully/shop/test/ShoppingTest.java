...
public class ShoppingTest {
	...
	protected void populateDB() throws Exception {
		Response response = client
				.target("http://localhost:8080/ex14_1/services/shop").request()
				.head();
	...
	public void testCreateOrder() throws Exception {
		...
		Customers custs = client.target(customers)
				...
				.queryParam("lastName", "Burke").request().get(Customers.class);
		Customer customer = null;
		if (custs.getCustomers().size() > 0) {
			System.out
					.println("- Found a Bill Burke in the database, using that");
			customer = custs.getCustomers().iterator().next();
		} else {
			System.out
					.println("- Cound not find a Bill Burke in the database, creating one.");
			...
			response = client.target(customers).request()
		...
		Products prods = client.target(products).queryParam("name", "iPhone")
				.request().get(Products.class);
