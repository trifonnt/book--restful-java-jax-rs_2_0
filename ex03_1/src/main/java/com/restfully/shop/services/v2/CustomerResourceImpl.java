...
// When overriding annotation metadata, then all metadata MUST be respecified!
//	@POST
//	@Consumes("application/xml;charset=utf-8")
	public Response createCustomer(InputStream inStream) {
		Customer customer = readCustomer( inStream );
		customer.setId(idCounter.incrementAndGet());
		customerDB.put(customer.getId(), customer);
		System.out.println("Created customer " + customer.getId());
		return Response
				.created(URI.create(AppConstants.CUSTOMERS_URL+"/" + customer.getId()))
				.build();
	}
	...
	public void updateCustomer(int id, InputStream inStream) {
		Customer update = readCustomer( inStream );
