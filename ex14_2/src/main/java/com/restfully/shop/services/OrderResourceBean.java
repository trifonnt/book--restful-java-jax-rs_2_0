...
@Transactional
public class OrderResourceBean implements IOrderResource {
	...
	public static LineItem entity2domain(LineItemEntity entity) {
		...
		Product product = ProductResourceBean
				.entity2domain(entity.getProduct());
	...
	public void purgeOrders() {
		...
		int updated = em.createQuery(
				"delete from PurchaseOrder o where o.cancelled = true")
				.executeUpdate();
	}
	...
	public Response getOrders(int start, int size, UriInfo uriInfo) {
		UriBuilder builder = uriInfo.getAbsolutePathBuilder();
		...
		List<OrderEntity> orderEntities = em.createQuery("select p from PurchaseOrder p").setFirstResult(start).setMaxResults(size).getResultList();
		...
		for (Object obj : orderEntities) {
			...
			URI self = uriInfo.getAbsolutePathBuilder()
					.path(Integer.toString(order.getId())).build();
			Link selfLink = Link.fromUri(self).rel("self")
					.type("application/xml").build();
			order.addLink(selfLink);
			if (!order.isCancelled()) {
				URI cancel = uriInfo.getAbsolutePathBuilder()
						.path(Integer.toString(order.getId())).path("cancel")
						.build();
				Link cancelLink = Link.fromUri(cancel).rel("cancel")
						.type("application/xml").build();
		...
		if (orderEntities.size() == size) {
			...
			Link nextLink = Link.fromUri(nextUri).rel("next")
					.type("application/xml").build();
			links.add(nextLink);
		}
		// previous link
		if (start > 0) {
			...
			Link previousLink = Link.fromUri(previousUri).rel("previous")
					.type("application/xml").build();
	...
	public Response getOrder(int id, UriInfo uriInfo) {
		...
		Link selfLink = Link.fromUri(self).rel("self").type("application/xml")
				.build();
		order.addLink(selfLink);
		if (!order.isCancelled()) {
			URI cancel = uriInfo.getAbsolutePathBuilder().path("cancel")
					.build();
			Link cancelLink = Link.fromUri(cancel).rel("cancel")
					.type("application/xml").build();
			order.addLink(cancelLink);
